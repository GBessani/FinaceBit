...
export function addFixedBill(data: FinancialData, bill: FixedBill): FinancialData {
  const newData = {
    ...data,
    fixedBills: [...data.fixedBills, bill],
  }
  saveData(newData)
  return newData
}
...
export function updateFixedBill(data: FinancialData, bill: FixedBill): FinancialData {
  const newData = {
    ...data,
    fixedBills: data.fixedBills.map((b) => (b.id === bill.id ? bill : b)),
  }
  saveData(newData)
  return newData
}
...
export function deleteFixedBill(data: FinancialData, id: string): FinancialData {
  const newData = {
    ...data,
    fixedBills: data.fixedBills.filter((b) => b.id !== id),
  }
  saveData(newData)
  return newData
}
...
export function addScheduledTransaction(data: FinancialData, transaction: ScheduledTransaction): FinancialData {
  const newData = {
    ...data,
    scheduledTransactions: [...data.scheduledTransactions, transaction],
  }
  saveData(newData)
  return newData
}
...
export function updateScheduledTransaction(data: FinancialData, transaction: ScheduledTransaction): FinancialData {
  const newData = {
    ...data,
    scheduledTransactions: data.scheduledTransactions.map((t) =>
      t.id === transaction.id ? transaction : t
    ),
  }
  saveData(newData)
...
export function deleteScheduledTransaction(data: FinancialData, id: string): FinancialData {
  const newData = {
    ...data,
    scheduledTransactions: data.scheduledTransactions.filter((t) => t.id !== id),
  }
  saveData(newData)
  return newData
}
